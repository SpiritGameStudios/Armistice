...
public class ArmisticeOutlinerProvider extends JsonCodecProvider<List<OutlinerNode>> {
	...
	public ArmisticeOutlinerProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider, ExistingFileHelper existingFileHelper) {
		super(output, PackOutput.Target.DATA_PACK, "model_bones", PackType.SERVER_DATA, OutlinerNode.CODEC.listOf(), lookupProvider, MODID, existingFileHelper);
	}
	...
	protected void gather() {
		...
		for (Map.Entry<ResourceLocation, Resource> entry : converter.listMatchingResources(manager).entrySet()) {
			...
			unconditional(id, modelData.getOrThrow().outliner());
		}
	}
}
